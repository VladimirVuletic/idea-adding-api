...
def create_idea(new_idea: IdeaCreate, ideas_repo: IdeasRepository = Depends(get_ideas_file_repo), git_client: GitClient = Depends(get_git_client)) -> Idea:
...
def update_idea(id: str, updated_idea: IdeaUpdate, ideas_repo: IdeasRepository = Depends(get_ideas_file_repo), git_client: GitClient = Depends(get_git_client)) -> Idea:
...
def delete_idea(id: str, ideas_repo: IdeasRepository = Depends(get_ideas_file_repo), git_client: GitClient = Depends(get_git_client)) -> Idea:
    deleted_idea = ideas_repo.delete_idea(id)

    if deleted_idea is None:
        raise HTTPException(status_code=404, detail=f"Project with id {id} not found.")

    git_client.push_changes(f"delete idea '{deleted_idea.name}' | id: {deleted_idea.id}")
